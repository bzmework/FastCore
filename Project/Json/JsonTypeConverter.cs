    ...
    internal static class JsonTypeConverter
    {
        ...
        private static Dictionary<string, Func<string, object>> TypeRuleFuncs
        {
            get
            {
                ...
                rules["int"] = new Func<string, object>(str => { return SerializerUtilities.FastStringToInt(str); });
                rules["long"] = new Func<string, object>(str => { return SerializerUtilities.FastStringToLong(str); });
                rules["double"] = new Func<string, object>(str => { return SerializerUtilities.FastStringToDouble(str); });
                return rules;
            }
        }
