...
namespace FastCore.Json
{
	...
	sealed class JsonMemberInfo
	{
		internal MemberInfo Member { get; set; }
		internal JsonPropertyAttribute Attribute { get; set; }
	}
	...
	internal static class SerializerUtilities
	{
		...
		{
			return Json.GetTypeIdentifierInstance(typeName);
		}
		...
		internal static DateTimeOffset FastStringToDateTimeoffset(string value, JsonSettings settings)
		{
			TimeSpan offset;
		...
		internal unsafe static string GetStringBasedValue(char* ptr, ref int index, JsonSettings settings)
		{
			...
			{
				current = ptr[index];
				if (count == 0 && current == settings.QuoteChar)
				{
					startIndex = index + 1;
					++count;
				}
				else if (count > 0 && current == settings.QuoteChar && prev != '\\')
				{
		...
		internal static string CustomEnumToStr(Enum @enum, JsonSettings settings)
		{
		...
		private static string DateToStringWithOffset(DateTime date, JsonSettings settings, TimeSpan offset)
		{
			return
				settings.HasDateStringFormat ? date.ToString(settings.DateStringFormat) :
				settings.DateFormat == JsonDateFormat.Default ? DateToString(date, settings, offset) :
				settings.DateFormat == JsonDateFormat.EpochTime ? DateToEpochTime(date) :
				DateToISOFormat(date, settings, offset);
		}

		private static string DateToISOFormat(DateTime date, JsonSettings settings, TimeSpan offset)
		{
			var timeZoneFormat = settings.TimeZoneFormat;
			...
			if (timeZoneFormat == JsonTimeZoneFormat.Utc)
				value.Append('Z');
			else if (timeZoneFormat == JsonTimeZoneFormat.Local)
			{
		...
		private static string DateToString(DateTime date, JsonSettings settings, TimeSpan offset)
		{
			...
			var hours = Math.Abs(offset.Hours);
			var minutes = Math.Abs(offset.Minutes);
			var offsetText = timeZoneFormat == JsonTimeZoneFormat.Local ? (string.Concat(offset.Ticks >= 0 ? "+" : "-", hours < 10 ? "0" : string.Empty,
				hours, minutes < 10 ? "0" : string.Empty, minutes)) : string.Empty;

			if (date.Kind == DateTimeKind.Utc && timeZoneFormat == JsonTimeZoneFormat.Utc)
			{
		...
		{
			Json.SetterPropertyValue(ref instance, value, methodInfo);
		}

		internal static void SetterFieldValue<T>(ref T instance, object value, FieldInfo fieldInfo)
		{
			Json.SetterFieldValue(ref instance, value, fieldInfo);
		}
	}
}
